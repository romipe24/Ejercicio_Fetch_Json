...
fetch(DATA_URL)
      .then(response => {
          if (response.ok) {
              return response.json(); //si hay respuesta la convierte en json
          } else {
              throw Error(`Error HTTP: ${response.status}`); //si no hay respuesta da error
          }
      })
      .then(data => {
      ...
      .catch(error => {
        ...
        container.innerHTML = `<p>Hubo un error al cargar los datos.</p>`;
      });
